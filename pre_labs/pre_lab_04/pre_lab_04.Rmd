...
Two changes that I notice after the refinr package is that it capitalized some of the variables within the column, such as Washington County Museum of Fine Arts to WASHINGTON COUNTY MUSEUM OF FINE ARTS. Another change that I notice is that it removed LLC from Rise Therapeutics and removed Inc. from the Melwood Horticultural Training Center. 
...
The first change I noticed is that it removed different words from similar variables so that they are the exact same. For example, WALTER P CARTER CHILD CARE CENTER INC was changed to	WALTER P CARTER CHILD CARE CENTER to match the others that have this name. The second change is that it lowercased Washington Adventist University after it was all uppercase. 
...
Compared to the previous use of key_collision_merge, I do not think that there are many substantial changes in the following code. 
